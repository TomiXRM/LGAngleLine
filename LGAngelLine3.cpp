...
void LGAngelLine::manage(VectorXY *DriveV,VectorXY *IRv,int16_t irDeg,int16_t imuDir,type_camera *camera){

        // X = xcosθ - ysinθ
        // Y = xsinθ + ycosθ

        if(isOnLine) {
                if((lineDepth - 5000) < 0 || isHalfOut || (abs(lineCurrentDegree) < 60 && IS_ONLINE_FIRSTTIME) ) {//|| lineWaitOnread_ms() > 1000
                        *DriveV = makeXYFromRT(100,lineDegree+180);
                        ...
                        if(camera->OwnW > 50 && camera->OwnY < 0) {//自陣
                                if(camera->OwnX < 0) {
                                        //右
                        ...
                        }else if(camera->OpsW > 50 && camera->OpsY > 0) {//相手
                                if(camera->OwnX < 0) {
                                        //右
        ...
        }
        {
                if (lineWait.read_ms() < 1000) {
                        ...
                        if( lineDirVector > 60 && DriveLCount > -20) {
