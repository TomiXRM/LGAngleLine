...
#define DEG_HISTORY_QTY 16
#define IS_ONLINE_FIRSTTIME lastOnLineFlag != isOnLine && isOnLine
#define IS_INFIELD_FIRSTTIME lastOnLineFlag != isOnLine && !isOnLine
//CONFIG
// #define DEBUG_ANGEL
class LGAngelLine {
public:
...
void lineConpute(int16_t imuDir);
void manage(VectorXY *DriveV,VectorXY *IRv,int16_t irDeg,int16_t imuDir,type_camera *camera);

void calcSlop();
void lineTrace(VectorXY *DriveV,int8_t speed);
...
int16_t lineFirstCenteredDegree;
int16_t lineDegree;
...
uint8_t clusterQty;
bool isHalfOut,isHalfOutPrev;
bool isOnLine;
bool lastOnLineFlag;
bool isOutsideLine;
int16_t errorDeg;
int16_t rotateX,lineDirVector;//座標回転フィルタリング

// VectorXY onLineFirstV;
VectorXY currentLine_CompassRotate;
int16_t DriveLCount,DriveNCount;
bool manageIgnore;
Timer lineWait;
Timer lineWaitOnLine;
Timer slowStart;
bool alpacoX,alpacoY;
std::deque<int16_t> degHistory;
